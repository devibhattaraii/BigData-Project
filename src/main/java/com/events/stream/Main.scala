...
object Main {
  ...
  def sendEvent(message: String) = {
    ...
    producer.send(new ProducerRecord[String, String](Config.module + "_out", key, message))
  ...
  def processRDD(rdd: RDD[String], sc: SparkContext) {
    ...
    if (!values.isEmpty && values.length == 4) {
      if (isNumeric(values(3))) {
        ...
        val page = values(1)
        val user = values(2)
        val dateTime = values(3)
        val str = action + "," + page + "," + user + "," + dateTime
        sendEvent(str)
        saveLogToHive(sc, action, page, user, dateTime)
      }
    }
  }
  ...
  def saveLogToHive(sc: SparkContext, action: String,page: String,  user: String,  datetime: String) {
    println("============== Start Saving Log Record  ================")
    val hiveContext = new HiveContext(sc)
    val sql: String = "select '" + action + "' as action, '" + datetime + "' as datetime, '" + page + "' as page, '" + user + "' as user"
